...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(strlen(trim($_POST["password"]??"")) == 0){ //正しく入力されているか確認
        $errors = "メールアドレスとパスワードを正しく入力して下さい。";
    }
    ...
    if(empty($errors)){
        ...
        try{
            ...
            $e -> getMessage(); //例外時にメッセージ
        }

        //ユーザ情報が取り出せた　かつ　パスワードが一致、セッションに値代入、掲示板へ遷移
        if($user && password_verify($input["password"],$user["password"])){
            ...
            //「ログイン状態を保持する」にチェック、セッションにセット
            if($_POST['login_keep'] == 1){
                $_SESSION['login_keep'] = $_POST['login_keep'];
            }
            //「ログイン状態を保持する」にチェック、クッキーセット、なければ削除
